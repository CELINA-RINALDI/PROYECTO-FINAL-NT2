<template>
    <div class="carga">
       ...
       <br>
       <p>Tipo:</p>
       <input type="radio" id="ahorro" name="tipo" value="ahorro" v-model="input.esAhorro">
       <label for="gasto">Ahorro</label><br>
          ...
          if (this.input.title != '' && this.input.amount != '') {
            ...
            if(elementoActivo) {
                if(elementoActivo.value == 'gasto') {
                    ...
                    this.input.esAhorro = false;
                    this.store.actualizarSegmento(this.input, 'b')
                        
                } else if (elementoActivo.value == 'ingreso') { 
                    this.input.esGasto = false;
                    this.input.esAhorro = false;
                    this.store.verificarAhorros(this.input);
                }else{
                    this.input.esAhorro = true;
                    if( this.store.saldoActual >= mov.amount) {
                        this.store.cargarAhorro(this.input);               
                    }else{
                        alert("No posees monto suficiente para transferir a ahorro");
                    }
                }
            } else {
